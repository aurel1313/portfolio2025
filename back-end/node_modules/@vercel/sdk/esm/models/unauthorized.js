/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import * as z from "zod";
import { remap as remap$ } from "../lib/primitives.js";
import { VercelError } from "./vercelerror.js";
export const Code = {
    Unauthorized: "unauthorized",
};
export const UnauthorizedTag = {
    Unauthorized: "Unauthorized",
};
export class Unauthorized extends VercelError {
    constructor(err, httpMeta) {
        const message = err.message || `API error occurred: ${JSON.stringify(err)}`;
        super(message, httpMeta);
        this.data$ = err;
        this.status = err.status;
        this.code = err.code;
        this.tag = err.tag;
        this.name = "Unauthorized";
    }
}
/** @internal */
export const Code$inboundSchema = z.nativeEnum(Code);
/** @internal */
export const Code$outboundSchema = Code$inboundSchema;
/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export var Code$;
(function (Code$) {
    /** @deprecated use `Code$inboundSchema` instead. */
    Code$.inboundSchema = Code$inboundSchema;
    /** @deprecated use `Code$outboundSchema` instead. */
    Code$.outboundSchema = Code$outboundSchema;
})(Code$ || (Code$ = {}));
/** @internal */
export const UnauthorizedTag$inboundSchema = z.nativeEnum(UnauthorizedTag);
/** @internal */
export const UnauthorizedTag$outboundSchema = UnauthorizedTag$inboundSchema;
/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export var UnauthorizedTag$;
(function (UnauthorizedTag$) {
    /** @deprecated use `UnauthorizedTag$inboundSchema` instead. */
    UnauthorizedTag$.inboundSchema = UnauthorizedTag$inboundSchema;
    /** @deprecated use `UnauthorizedTag$outboundSchema` instead. */
    UnauthorizedTag$.outboundSchema = UnauthorizedTag$outboundSchema;
})(UnauthorizedTag$ || (UnauthorizedTag$ = {}));
/** @internal */
export const Unauthorized$inboundSchema = z.object({
    status: z.number(),
    code: Code$inboundSchema,
    message: z.string(),
    _tag: UnauthorizedTag$inboundSchema,
    request$: z.instanceof(Request),
    response$: z.instanceof(Response),
    body$: z.string(),
})
    .transform((v) => {
    const remapped = remap$(v, {
        "_tag": "tag",
    });
    return new Unauthorized(remapped, {
        request: v.request$,
        response: v.response$,
        body: v.body$,
    });
});
/** @internal */
export const Unauthorized$outboundSchema = z.instanceof(Unauthorized)
    .transform(v => v.data$)
    .pipe(z.object({
    status: z.number(),
    code: Code$outboundSchema,
    message: z.string(),
    tag: UnauthorizedTag$outboundSchema,
}).transform((v) => {
    return remap$(v, {
        tag: "_tag",
    });
}));
/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export var Unauthorized$;
(function (Unauthorized$) {
    /** @deprecated use `Unauthorized$inboundSchema` instead. */
    Unauthorized$.inboundSchema = Unauthorized$inboundSchema;
    /** @deprecated use `Unauthorized$outboundSchema` instead. */
    Unauthorized$.outboundSchema = Unauthorized$outboundSchema;
})(Unauthorized$ || (Unauthorized$ = {}));
//# sourceMappingURL=unauthorized.js.map